...
docker run -it \
  ...
  postgres:13
```
docker run -d -e POSTGRES_USER="root" -e POSTGRES_PASSWORD="root" -e POSTGRES_DB="ny_taxi" -v /Users/batman/Learning/ZoomCamp/1.1/ny_taxi_postgres_data:/var/lib/postgresql/data -p 5432:5432 --network pg-network --name pg-database postgres:13
...
docker run -d \
  ...
  taxi_ingest:v001 \
    ...
    --url=${URL} 
```
docker run -d   --network=pg-network   taxi_ingest:v001     --user=root     --password=root     --host=pg-database     --port=5432     --db=ny_taxi     --table_name=yellow_taxi_trips  --url=${URL}
